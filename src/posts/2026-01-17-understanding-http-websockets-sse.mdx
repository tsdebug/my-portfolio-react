...
*   **Request** - You (client) write a letter asking for some information and mail it to the server.
*   **Response** - Server gets your letter, finds the needed information and then mails you a letter back.
*   **Close** - That’s all, then the connection closes. The server completely forgets about you as it didn’t maintain any kind of database to store data about you (this is why http is called ‘stateless’).
...
1.  **Scalability (The biggest reason):** Assume Facebook has 1 billion users— If HTTP were state-FULL: The server would have to keep a file open in the RAM for every single user to keep a record of the individuals’ actions. Can you imagine the load? That’s a lot, a billion open files can crash even the biggest supercomputer in the world! But since HTTP are state-LESS: The server answers to your request and instantly clears up the RAM. It doesn’t carry the ‘baggage’ of remembering you and your actions.
2.  **Simplicity:** In case of any problem with the connection mid-transaction the server doesn’t need to panic about things like ‘restoring the session’, the client (you) can send the request again. Just that simple!
3.  **Load Balance:** Imagine you have two servers - Request1 > Server A ; Request2 > Server B. We know how forgetful HTTP is? Every transaction is brand new, thus Server2 doesn’t have to ask Server1 about anything like “Hey who’s this guy?”. The servers can perform independently with requests, hence Server B can process the request on its own. 
...
5.  **Step 5: How to read the Message tab**
    *   **The Colours:** You will see a table with ‘Data’, ‘Length’ and ‘Time’. In the ‘Data’ column if **Green Upwards Arrow** - You sent the data to the server. If **Red Downwards Arrow** - Server sent data to you.
    *   **The Heartbeat:** The "Phone Line" will be cut by the internet providers if it is silent for too long. **Ping:** The Server asks, "Are you still there?" **Pong:** The Browser replies, "Yes, don't hang up." This is automatic.
...
*   **Request:** You sent a normal HTTP GET request (as usual).
*   **Response:** Server goes, “Here is your file..” BUT for case of SSE, it sets the content-type to text/event-stream.
*   **Trick behind:** The server never sends the ‘End of file’ signal and it’s like keeping the letter open. It keeps the letter open and writes a new line in the bottom forever → a never ending letter kind of.
...
**Use SSE (Server Sent Events): (The "Radio Station”)**
...
You can follow the Official Mermaid js doc for more details and updates. 
And it actually gets better when you get to know that no complex installation is required, you can simply try it out on its playground called Mermaid Live Editor. On the left side you write the code and on the right the visuals appear, and are updated in real-time thus no need to save changes every time.

**Here is the cool part:**
*   **Sample Diagrams:** In the editor, there is a "Sample" button. You can click it to instantly load Flowcharts, Pie Charts, State Diagrams, and more to see how they work.
*   **Themes:** Don't like the default look? You can change the "Theme" in the settings (to Dark Mode, Forest, or Neutral) without changing a single line of code.
...
**A sample code that you can try right now! Go to Mermaid Live Editor and paste it in the code section:**
...
![Mermaid js interaction sequence diagram](./blog-assets/md.png)
</div>
